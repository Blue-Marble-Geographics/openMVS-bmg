	...
	namespace CUDA {
		...
		void PatchMatch::EstimateDepthMap(DepthData& depthData)
		{
			...
			for (unsigned scaleNumber = totalScaleNumber + 1; scaleNumber-- > 0; ) {
				...
				// surface a block-height clamp once: it means this GPU could not launch the
				// kernel at the compiled-for size, so the estimator stepped down rather than
				// failing (which the caller would have reported as "switching to CPU-only")
				{
					static int sReportedBlockH = 0;
					if (sReportedBlockH != g_pmcudaEffectiveBlockH) {
						sReportedBlockH = g_pmcudaEffectiveBlockH;
						if (g_pmcudaEffectiveBlockH != PMCUDA_BLOCK_H)
							VERBOSE("CUDA: PatchMatch block height reduced %d -> %d to fit this device's per-block register budget",
