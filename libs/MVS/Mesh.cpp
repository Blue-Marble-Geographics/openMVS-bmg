...
void Mesh::ListIncidentVertices()
{
	...
	FOREACH(i, faces) {
		...
		for (int v=0; v<3; ++v) {
			...
			for (int i=1; i<3; ++i) {
				const VIndex idxVert(face[SmallMod3(v+i)]);
...
void Mesh::ListIncidentFaceFaces()
{
	...
	FOREACH(f, faces) {
		...
		for (int v=0; v<3; ++v) {
			...
			const FaceIdxArr& facesJ = *pFaces[SmallMod3(v+1)];
...
void Mesh::CloseHole(VertexIdxArr& split0)
{
	...
	if (split0.size() == 3) {
		...
		for (int v=0; v<3; ++v) {
			#ifndef _RELEASE
			FaceIdxArr indices;
			GetAdjVertexFaces(split0[v], split0[SmallMod3(v+1)], indices);
			ASSERT(indices.size() < 2);
			indices.Empty();
			GetAdjVertexFaces(split0[v], split0[SmallMod3(v+2)], indices);
			ASSERT(indices.size() < 2);
			#endif
...
void Mesh::CloseHoleQuality(VertexIdxArr& verts)
{
	...
	while(true) {
		...
		for (int v=0; v<3; ++v) {
			#ifndef _RELEASE
			FaceIdxArr indices;
			GetAdjVertexFaces(candidateFace[v], candidateFace[SmallMod3(v+1)], indices);
			ASSERT(indices.size() < 2);
			indices.Empty();
			GetAdjVertexFaces(candidateFace[v], candidateFace[SmallMod3(v+2)], indices);
			ASSERT(indices.size() < 2);
			#endif
