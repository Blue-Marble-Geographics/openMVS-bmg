    ...
    void reserve(size_type n)
    {
      ...
      do
      {
        ...
        //put_block_on_free_list(new_block, block_size - 2);
        {
          ...
          // will correspond to the iterator order...
#if 1
// Assume: blockSizeUsable is the count of usable nodes (excludes sentinels).
          pointer oldHead = free_list;
          pointer base = new_block;
          auto blockSizeUsable = block_size;

          // Link nodes [1 .. blockSizeUsable - 1] to their successors in parallel.
#pragma omp parallel for schedule(static)
          for (ptrdiff_t i = 1; i <= static_cast<ptrdiff_t>(blockSizeUsable-2) - 1; ++i) {
            pointer it = base + i;
            Traits::set_pointer(
              *it,
              reinterpret_cast<void*>(
                reinterpret_cast<std::uintptr_t>(
                  clean_pointer(reinterpret_cast<char*>(it + 1))
                  ) | 2
                )
            );
          }

          // Tail: node[blockSizeUsable] points to old head.
          {
            pointer it = base + static_cast<ptrdiff_t>(blockSizeUsable-2);
            Traits::set_pointer(
              *it,
              reinterpret_cast<void*>(
                reinterpret_cast<std::uintptr_t>(
                  clean_pointer(reinterpret_cast<char*>(oldHead))
                  ) | 2
                )
            );
          }

          // Publish head (node[1]) as new free_list.
          free_list = base + 1;

#else
          for (size_type i = block_size-2; i >= 1; --i)
          {
            //put_on_free_list(new_block + i);
            //set_type(new_block+i, free_list, FREE);
            char* p_cleaned = clean_pointer(reinterpret_cast<char*>(free_list));
            std::ptrdiff_t p_value = reinterpret_cast<std::ptrdiff_t>(p_cleaned);
            std::ptrdiff_t new_ptr = p_value + static_cast<std::ptrdiff_t>(FREE);
            Traits::set_pointer(*(new_block+i), reinterpret_cast<void*>(new_ptr));
...
public:
  ...
  void reserve(size_type n)
  {
    ...
    while (capacity_ < n) {
      ...
      set_type(last_item, nullptr, START_END);
      // JPB WIP BUG Increment_policy::increase_size(*this);
    }
...
#pragma omp parallel for schedule(static)
   ...
   //     *(it+1) = reinterpret_cast<void*>((reinterpret_cast<std::uintptr_t>(it + 2) & ~3ull) | 2ull);
    ...
    for (size_type i = 0; i < localHeads.size(); ++i) {
      pointer head = localHeads[i];
      pointer tail = localTails[i];
      if (!head || !tail) continue;

      set_type(tail, free_list, FREE);
      free_list = head;
    }
  }
#endif
